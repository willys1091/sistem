@php
    ...
    $purchaseViewRequestPermission = user()->permission('view_purchase_request');
    $purchaseViewQuotationPermission = user()->permission('view_quotation');
...
|| $purchaseViewCreditPermission != 'none' || $purchaseViewInventoryPermission != 'none' || $purchaseViewOrderReportPermission != 'none' || $purchaseViewPaymentPermission != 'none'))
    ...
    <x-menu-item icon="wallet" :text="__('purchase::app.menu.purchase')" :addon="App::environment('demo')">
        ...
        <div class="accordionItemContent pb-2">
            ...
            <x-sub-menu-item :link="route('purchase-request.index')" :text="__('purchase::app.menu.purchaseRequest')" :permission="($purchaseViewRequestPermission != 'none' && $purchaseViewRequestPermission != '')"/>

            <x-sub-menu-item :link="route('quotation.index')" :text="__('purchase::app.menu.quotation')" :permission="($purchaseViewQuotationPermission != 'none' && $purchaseViewQuotationPermission != '')"/>
            ...
            <x-sub-menu-item :link="route('reports.index')" :text="__('purchase::app.menu.reports')" :permission="($purchaseViewOrderReportPermission != 'none' && $purchaseViewOrderReportPermission != '')"/>
        </div>
    </x-menu-item>
@endif