    ...
    <div class="content-wrapper">
        <div class="d-flex justify-content-between action-bar">
            <div id="table-actions" class="flex-grow-1 align-items-center mt-3">
                ...
                <x-forms.button-primary class="mr-3 float-left" id="add-header" icon="plus" data-redirect-url="{{ route('quotation.index') }}">
                    @lang('app.add') @lang('app.quotation')
...
@push('scripts')
    ...
    <script>
        $('body').on('click', '#add-header', function() {
            ...
            var url = "{{ route('quotation.create') }}?redirectUrl="+redirectUrl;
        ...
        $('#quotation-table').on('preXhr.dt', function(e, settings, data) {
        ...
        const showTable = () => {
            window.LaravelDataTables["quotation-table"].draw();
        }
        ...
        const applyQuickAction = () => {
            var rowdIds = $("#quotation-table input:checkbox:checked").map(function() {
                return $(this).val();
            }).get();
        ...
        $('body').on('click', '.delete-table-row', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('quotation.destroy', ':id') }}";
                    url = url.replace(':id', id);
        ...
        $('body').on('click', '.sendButton', function() {
            ...
            var url = "{{ route('quotation.send_request', ':id') }}";
            url = url.replace(':id', id);
        ...
        $('body').on('click', '.open-approval', function() {
            ...
            var url = "{{ route('quotation.approval_list', ':id') }}";
            url = url.replace(':id', id);
            $(MODAL_LG + ' ' + MODAL_HEADING).html('...');
        ...
        $('body').on('click', '.open-response', function() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('quotation.check_approval') }}",
                data: {'_token': "{{ csrf_token() }}",actid: actid},
                cache: false,
                success: function(response){
                    if (response.status == "success") {
                        var url = "{{ route('quotation.response', [':id', ':actid']) }}";
        ...
        $('body').on('click', '.copy', function() {
            ...
            $.easyAjax({
                ...
                type: "POST",
                data: {'_token': "{{ csrf_token() }}",quotationId: id},
                success: function(response) {
                    if (response.status == "success") {
                        showTable();
                    }
        ...
        $('body').on('change', '#delivery-status', function() {
            ...
            let value = $(this).val();
            let url = "{{route('quotation.change_status', ':id')}}";
            url = url.replace(':id', id);
