...
class FinanceExecutorsReimbursementsDataTable extends BaseDataTable{
    use General;
    ...
    public function dataTable($query){
        ...
        $datatables->addColumn('action', function ($row) {
            ...
            $action .= '<a class="task_view_more d-flex align-items-center justify-content-center text-success paid" href="javascript:;" data-reimbursement-id="' . $row->id . '"><i class="fa fa-check mr-2"></i></a>';
            $action .= '</div>';
            return $action;
        });
    ...
    public function query(){
        ...
        $model = Reimbursement::with('currency', 'user', 'user.employeeDetail', 'user.employeeDetail.designation', 'user.session')
            ->select('reimbursements.id','reimbursements.code', 'reimbursements.project_id', 'reimbursements.item_name', 'reimbursements.user_id', 'reimbursement_approval.user_id as approval_user_id', 'reimbursements.price', 'users.salutation', 'users.name', 'reimbursements.purchase_date', 'reimbursements.currency_id', 'currencies.currency_symbol', 'reimbursements.status','reimbursements.state_id', 'reimbursements.purchase_from', 'reimbursements.reimbursements_recurring_id', 'designations.name as designation_name', 'reimbursements.added_by', 'projects.deleted_at as project_deleted_at', 'reimbursements.is_copied','approval_state.name as state_name')
        ...
        $model->whereRaw("`reimbursements`.`status`= 'approved'");
