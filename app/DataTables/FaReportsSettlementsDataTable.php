...
class FaReportsSettlementsDataTable extends BaseDataTable{
    use General;
    ...
    public function dataTable($query){
        ...
        $datatables->editColumn('item_name', function ($row) {
            if (is_null($row->settlements_recurring_id)) {
                return '<a href="' . route('settlements.show', [$row->id]) . '" class="text-darkest-grey">' . $row->item_name . '</a>';
            }
            return '<a href="' . route('settlements.show', [$row->id]) . '" class="text-darkest-grey">' . $row->item_name . '</a>
                    <p class="mb-0">
                        <span class="badge badge-primary">' . __('app.recurring') . '</span>
                    </p>';
        });
    ...
    public function query(){
        ...
        $model->whereRaw("`settlements`.`status` in ('on progress','approved','rejected')");
    ...
    protected function getColumns(){
        $data = [
            ...
            __('modules.settlements.urgency') => ['data' => 'urgency', 'name' => 'urgency', 'title' => __('modules.settlements.urgency')],
