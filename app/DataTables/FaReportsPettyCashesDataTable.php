...
class FaReportsPettyCashesDataTable extends BaseDataTable{
    use General;
    ...
    public function dataTable($query){
        ...
        $datatables->editColumn('item_name', function ($row) {
            if (is_null($row->pettycashes_recurring_id)) {
                return '<a href="' . route('pettycashes.show', [$row->id]) . '" class="text-darkest-grey">' . $row->item_name . '</a>';
            }
            return '<a href="' . route('pettycashes.show', [$row->id]) . '" class="text-darkest-grey">' . $row->item_name . '</a>
    ...
    public function query(){
        ...
        ->select('pettycashes.id','pettycashes.code','pettycashes.project_id','pettycashes.item_name','pettycashes.user_id','pettycash_approval.user_id as approval_user_id','pettycashes.urgency','pettycashes.price','users.salutation','users.name','pettycashes.purchase_date','pettycashes.currency_id','currencies.currency_symbol','pettycashes.status','pettycashes.state_id','pettycashes.purchase_from','pettycashes.pettycashes_recurring_id','designations.name as designation_name','pettycashes.added_by','projects.deleted_at as project_deleted_at','pettycashes.is_copied','approval_state.name as state_name','pettycashes.is_detail')
        ...
        //     $model = $model->where('pettycashes.pettycashes', '=', $request->recurringID);
        // }
        ...
        $model->whereRaw("`pettycashes`.`status` in ('on progress','approved','rejected')");
    ...
    protected function getColumns(){
        $data = [
            ...
            // __('modules.pettycashes.urgency') => ['data' => 'urgency', 'name' => 'urgency', 'title' => __('modules.pettycashes.urgency')],
            ...
            __('app.pettycashes') . ' ' . __('app.status') => ['data' => 'status_export', 'name' => 'status', 'visible' => false, 'title' => __('app.pettycashes')]
        ];
