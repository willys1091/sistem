...
class FaReportsReimbursementsDataTable extends BaseDataTable{
    use General;
    ...
    public function dataTable($query){
        ...
        $datatables->editColumn('item_name', function ($row) {
            if (is_null($row->reimbursements_recurring_id)) {
                return '<a href="' . route('reimbursements.show', [$row->id]) . '" class="text-darkest-grey">' . $row->item_name . '</a>';
            }
            return '<a href="' . route('reimbursements.show', [$row->id]) . '" class="text-darkest-grey">' . $row->item_name . '</a>
    ...
    public function query(){
        ...
        $model = Reimbursement::with('currency','user','user.employeeDetail','user.employeeDetail.designation','user.session')
                ...
                //     });
                // }
        $model->whereRaw("`reimbursements`.`status` in ('on progress','approved','rejected')");
