        ...
        $datatables->addColumn('action', function ($row) {
            ...
            if (!is_null($row->end_time)) {
                if ($this->approveTimelogPermission == 'all') {
                    if (!$row->approved) {
                        $action .= '<a class="dropdown-item approve-timelog text-success" href="javascript:;" data-time-id="' . $row->id . '">
                                <i class="fa fa-check mr-2"></i>' . trans('app.approve') . '</a>';
                        $action .= '<a class="dropdown-item reject-timelog text-danger" href="javascript:;" data-time-id="' . $row->id . '">
                                <i class="fa fa-times mr-2"></i>' . trans('app.reject') . '</a>';
                    }
                }
                if ($this->editTimelogPermission == 'all' || ($row->project_admin == user()->id) || ($this->editTimelogPermission == 'added' && $row->added_by == user()->id) || ($this->editTimelogPermission == 'owned' && (($row->project && $row->project->client_id == user()->id) || $row->user_id == user()->id)) || ($this->editTimelogPermission == 'both' && (($row->project && $row->project->client_id == user()->id) || $row->user_id == user()->id || $row->added_by == user()->id))) {
                ...
                if ($this->deleteTimelogPermission == 'all' || ($this->deleteTimelogPermission == 'added' && user()->id == $row->added_by) || ($row->project_admin == user()->id)) {
        ...
        $datatables->editColumn('total_hours', function ($row) {
            if (is_null($row->end_time)) {
                ...
                if ($row->approved=='1') {
                    ...
                    $timeLog .= ' <i data-toggle="tooltip" data-original-title="' . __('app.reject') . '" class="fa fa-times-circle text-danger"></i>';
                }
            }
