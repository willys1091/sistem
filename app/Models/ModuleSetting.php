...
class ModuleSetting extends BaseModel
{
    ...
    public static function addCompanyIdToNullModule($company, $module){
        ...
        if ($company->id == 1) {
            ModuleSetting::withoutGlobalScope(CompanyScope::class)->where('module_name', $module)->whereNull('company_id')->update(['company_id' => $company->id]);
        }
    }
    ...
    public static function createRoleSettingEntry($module, $roles, $company){
        ...
        foreach ($roles as $role) {
            $data = ModuleSetting::withoutGlobalScope(CompanyScope::class)->where('module_name', $module)->where('type', $role)->where('company_id', $company->id)->first();
