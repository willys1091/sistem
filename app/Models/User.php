...
class User extends BaseModel{
    ...
    public function expenseApproval(): HasMany{
        return $this->HasMany('App\Models\ExpenseApproval');
    }
    ...
    public static function allEmployees($exceptId = null, $active = false, $overRidePermission = null, $companyId = null){
        ...
        if (!isRunningInConsoleOrSeeding() && user()) {
            ...
            if (in_array('client', user_roles())) {
                $clientEmployees = Project::where('client_id', user()->id)->join('project_members', 'project_members.project_id', '=', 'projects.id')->select('project_members.user_id')->get()->pluck('user_id');
    ...
    public static function departmentUsers($teamId){
        $users = User::join('employee_details', 'employee_details.user_id', '=', 'users.id')
            ->select('users.id', 'users.name', 'users.email', 'users.created_at')->where('employee_details.department_id', $teamId);
        return $users->get();
    }
    ...
    public static function firstSuperAdmin(){
        return User::withoutGlobalScopes(['active', CompanyScope::class])->where('is_superadmin', 1)->whereNull('company_id')->orderBy('id')->first();
    }

    public static function allSuperAdmin(){
        return User::withoutGlobalScopes(['active', CompanyScope::class])->where('is_superadmin', 1)->whereNull('company_id')->get();
    }
