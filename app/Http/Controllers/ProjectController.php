...
class ProjectController extends AccountBaseController{
    ...
    public function archiveDestroy($id){
        Project::destroy($id);
        $this->logProjectActivity($id, user()->name . ' Archive this Project');
        return Reply::success(__('messages.projectArchiveSuccessfully'));
    }
    ...
    public function updateStatus(Request $request, $id){
        Project::findOrFail($id)->update(['status' => $request->status,]);
        return Reply::success(__('messages.updateSuccess'));
    }
    ...
    public function edit($id){
        $this->project = Project::with('client', 'members', 'members.user', 'members.user.session', 'members.user.employeeDetail.designation', 'milestones', 'milestones.currency')
            ->withTrashed()->findOrFail($id)->withCustomFields();
    ...
    public function show($id){
        ...
        default:
            ...
            $this->taskBoardStatus = TaskboardColumn::all();
            $this->earnings = Payment::where('status', 'complete')->where('project_id', $id)->sum('amount');
            $this->hoursLogged = intdiv($hoursLogged - $breakMinutes, 60).'.'.(($hoursLogged - $breakMinutes) % 60);
            $this->expenses = Expense::where(['project_id' => $id])->sum('price');
        ...
        $this->logProjectActivity($id, user()->name . ' Open this Project');
    ...
    public function amountBudgetChartData($project){
        ...
        $earnings = Payment::where('status', 'complete')->where('project_id', $project->id)->sum('amount');
        ...
        if($request->has('task')){
            ...
            if ($projectTasks) {
                foreach ($projectTasks as $projectTask) {
                    ...
                    $task->saveQuietly();
                    $this->logProjectActivity($request->duplicateProjectID, user()->name . ' Duplicate Project');
