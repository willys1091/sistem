...
class TaskBoardController extends AccountBaseController{
    ...
    public function update(UpdateTaskBoard $request, $id){
        ...
        if($request->has('before')){
            TaskboardColumn::where('priority', '<', $oldPosition)->where('priority', '>=', $newPosition)->orderBy('priority', 'asc')->increment('priority');
            $board->priority = $request->priority;
        }elseif($oldPosition > $newPosition){
            TaskboardColumn::where('priority', '<', $oldPosition)->where('priority', '>', $newPosition)->orderBy('priority', 'asc')->increment('priority');
            $board->priority = $request->priority + 1;
        }else{
            TaskboardColumn::where('priority', '>', $oldPosition)->where('priority', '<=', $newPosition)->orderBy('priority', 'asc')->decrement('priority');
            $board->priority = $request->priority;
        }
    ...
    public function destroy($id){
        ...
        $board = TaskboardColumn::findOrFail($id);
        $otherColumns = TaskboardColumn::where('priority', '>', $board->priority)->orderBy('priority', 'asc')->get();
