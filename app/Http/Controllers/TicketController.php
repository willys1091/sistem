...
class TicketController extends AccountBaseController{
    ...
    public function show($ticketNumber){
        $this->ticket = Ticket::with('project')->where('ticket_number', $ticketNumber)->firstOrFail();
    ...
    public function updateOtherData(Request $request, $id){
        ...
        $ticket->channel_id = $request->channel_id;
        $ticket->status = $request->status;
        $agentGroupData = TicketAgentGroups::where('company_id', company()->id)->where('status', 'enabled')->where('group_id', request()->group_id)->pluck('agent_id')->toArray();
        $ticketData = $ticket->where('company_id', company()->id)->where('group_id', request()->group_id)->whereIn('agent_id', $agentGroupData)->whereIn('status', ['open', 'pending'])->whereNotNull('agent_id')->pluck('agent_id')->toArray();
