...
class TimelogController extends AccountBaseController{
    ...
    public function create(){
        ...
        if (request()->has('default_assign') && request('default_assign') != '') {
            ...
            })->orWhere('projects.public', 1)->orderBy('project_name', 'asc')->get();
    ...
    public function store(StoreTimeLog $request){
        ...
        $activeTimer = ProjectTimeLog::with('user')->where(function ($query) use ($startDateTime, $endDateTime) {
                $query->where(
            ...
            })->join('users', 'users.id', '=', 'project_time_logs.user_id')->where('user_id', $userID)->first();
    ...
    public function destroy($id){
        ...
        abort_403(!($deleteTimelogPermission == 'all'|| ($deleteTimelogPermission == 'added' && $timelog->added_by == user()->id)|| ($timelog->project && ($timelog->project->project_admin == user()->id))));
    ...
    public function showTimer(){
        ...
        $activeTimer = ProjectTimeLog::with('user')
            ->whereNull('end_time')->join('users', 'users.id', '=', 'project_time_logs.user_id')->where('user_id', $this->user->id)->first();

        if (is_null($activeTimer)) {
            $this->tasks = Task::join('task_users', 'task_users.task_id', '=', 'tasks.id')
                ->with('project')->pending()->where('task_users.user_id', '=', $this->user->id)->select('tasks.*')->get();
    ...
    public function startTimer(StartTimer $request){
        ...
        if (is_null($activeTimer) || (!is_null($activeTimer->activeBreak))) {
            ...
            $this->activeTimerCount = ProjectTimeLog::whereNull('end_time')
                ->join('users', 'users.id', '=', 'project_time_logs.user_id')->select('project_time_logs.id');
    ...
    public function stopTimer(StopTimer $request){
        ...
        $selfActiveTimer = ProjectTimeLog::doesnthave('activeBreak')->where('user_id', user()->id)->whereNull('end_time')->first();
    ...
    public function showActiveTimer(){
        ...
        $this->myActiveTimer = ProjectTimeLog::with('task', 'task.project', 'user', 'project', 'breaks', 'activeBreak')->where('user_id', user()->id)->whereNull('end_time')->first();
        ...
        $this->tasks = Task::join('task_users', 'task_users.task_id', '=', 'tasks.id')->with('project')->pending()->where('task_users.user_id', '=', $this->user->id)->select('tasks.*')->get();
        $this->projects = Project::byEmployee(user()->id);
    ...
    public function employeeData(Request $request){
        ...
        $this->employees = $this->employees->groupBy('project_time_logs.user_id')->orderBy('users.name')->get();
    ...
    public function userTimelogs(Request $request){
        ...
        $this->timelogs = ProjectTimeLog::with('project', 'task')->select('*')->whereBetween('start_time', [$startDate, $endDate])->whereNotNull('end_time')->where('user_id', $employee);

        if ($projectId != 'all') {
            $this->timelogs = $this->timelogs->where('project_id', $projectId);
        }
        $this->timelogs = $this->timelogs->orderByDesc('end_time')->get();
