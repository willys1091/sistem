...
class RolePermissionController extends AccountBaseController{
    ...
    public function index(){
        ...
        $this->roles = Role::withCount('users')->orderBy('id', 'asc')->get();
    ...
    public function store(Request $request){
        ...
        $permissionRole = PermissionRole::where('permission_id', $permissionId)->where('role_id', $roleId)->first();

        if ($permissionRole) {
            $permissionRole = PermissionRole::where('permission_id', $permissionId)->where('role_id', $roleId)->update(['permission_type_id' => $permissionType]);
        }else {
    ...
    public function updateUserPermissions($roleId, $userId){
        ...
        foreach ($rolePermissions as $key => $value) {
            UserPermission::where('permission_id', $value->permission_id)->where('user_id', $userId)->update(['permission_type_id' => $value->permission_type_id]);
        }
    ...
    public function permissionRole($allPermissions, $type, $companyId){
        $role = Role::withoutGlobalScope(CompanyScope::class)->with('roleuser', 'roleuser.user.roles')->where('name', $type)->where('company_id', $companyId)->first();
        PermissionRole::where('role_id', $role->id)->delete();
        ...
        PermissionRole::whereIn('permission_id', $permissions->pluck('id')->toArray())->where('role_id', $role->id)->delete();
