...
class CompanyObserver{
    ...
    public function expensesCategory($company){
        $cat = new ExpensesCategory;
        $cat->company_id = $company->id;
        $cat->category_name = 'Kekaryawanan';
        $cat->is_detail = '1';
        $cat->saveQuietly();

        $role = new ExpensesCategoryRole;
        $role->company_id = $company->id;
        $role->expenses_category_id = $cat->id;
        $role->role_id = Role::whereCompany_idAndName($company->id,'employee')->value('id');
        $role->saveQuietly();

        $cat = new ExpensesCategory;
        $cat->company_id = $company->id;
        $cat->category_name = 'Non-Kekaryawanan';
        $cat->is_detail = '1';
        $cat->saveQuietly();
    ...
    public function roles($company): void{
        ...
        $clientRole->description = 'Client can see own tasks and projects.';
        $clientRole->saveQuietly();

        $clientRole = new Role();
        $clientRole->name = 'finance-checker';
        $clientRole->company_id = $company->id;
        $clientRole->display_name = 'Finance Checker';
        $clientRole->description = '';
        $clientRole->saveQuietly();

        $clientRole = new Role();
        $clientRole->name = 'accounting';
        $clientRole->company_id = $company->id;
        $clientRole->display_name = 'Accounting';
        $clientRole->description = '';
        $clientRole->saveQuietly();

        $clientRole = new Role();
        $clientRole->name = 'finance-executor';
        $clientRole->company_id = $company->id;
        $clientRole->display_name = 'Finance Executor';
        $clientRole->description = '';
        $clientRole->saveQuietly();
    ...
    public function unitType($company): void{
        $unitTypes = [
            ['unit_type' => 'Pcs','unit_type2' => 'Pcs','unit_type3' => 'Pcs', 'default' => 1, 'company_id' => $company->id],
            ['unit_type' => 'Unit','unit_type2' => 'Unit','unit_type3' => 'Unit', 'default' => 0, 'company_id' => $company->id],
            ['unit_type' => 'Pack','unit_type2' => 'Pack','unit_type3' => 'Pack', 'default' => 0, 'company_id' => $company->id],
            ['unit_type' => 'Box','unit_type2' => 'Box','unit_type3' => 'Box', 'default' => 0, 'company_id' => $company->id],
            ['unit_type' => 'Lusin','unit_type2' => 'Lusin','unit_type3' => 'Lusin', 'default' => 0, 'company_id' => $company->id],
            ['unit_type' => 'Kg','unit_type2' => 'Kg','unit_type3' => 'Kg', 'default' => 0, 'company_id' => $company->id],
            ['unit_type' => 'Gram','unit_type2' => 'Gram','unit_type3' => 'Gram', 'default' => 0, 'company_id' => $company->id],
        ];
    ...
    public static function widgetUpdate($company, $moduleInPackage){
        $modulesAll = Module::where('module_name', '<>', 'settings')->where('module_name', '<>', 'dashboards')->whereNotIn('module_name', Module::disabledModuleArray())->get();
        ...
        foreach ($widgets as $widget) {
            foreach ($widget as $key => $widgetValue) {
                if (in_array($key, $missingModules)) {
                    ...
                    DashboardWidget::whereIn('widget_name', $widgetValue)->where('company_id', $company->id)->where('dashboard_type', 'private-dashboard')->update(['active' => 0]);
                }
                elseif (in_array($key, $commonModules)) {
                    ...
                    DashboardWidget::whereIn('widget_name', $widgetValue)->where('company_id', $company->id)->where('dashboard_type', 'private-dashboard')->update(['active' => 1]);
                }
            }
        }
    }
