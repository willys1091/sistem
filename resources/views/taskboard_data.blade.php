@foreach ($result['boardColumns'] as $key => $column)
    <div class="minimized rounded bg-additional-grey border-grey mr-3 d-none column-mini-{{ $column->id }}">
        <div class="d-flex mt-4 mx-1 b-p-header align-items-center">
            ...
            <span class="b-p-badge bg-grey f-13 px-2 py-2 text-lightest font-weight-bold rounded d-inline-block">{{ $column->tasks_count }}</span>
        </div>
    </div>
    <div class="board-panel rounded bg-additional-grey border-grey mr-3 column-max-{{ $column->id }}">
        <div class="d-flex m-3 b-p-header">
            <p class="mb-0 f-15 mr-3 text-dark-grey font-weight-bold"><i class="fa fa-circle mr-2 text-yellow" style="color: {{ $column->label_color }}"></i>{{ $column->slug == 'completed' || $column->slug == 'incomplete' ? __('app.' . $column->slug) : $column->column_name }}</p>
            <span class="b-p-badge bg-grey f-13 px-2 text-lightest font-weight-bold rounded d-inline-block">{{ $column->tasks_count }}</span>
