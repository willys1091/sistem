...
@section('filter-section')
    <div class="d-flex d-lg-block filter-box project-header bg-white">
        ...
        <div class="project-menu" id="mob-client-detail">
            ...
            @if(isset($reimbursement->id))
                <nav class="tabs">
                    <ul class="-primary">
                        <li><x-tab :href="route('reimbursements.show', $reimbursement->id)" :text="__('modules.reimbursements.overview')" class="overview"/></li>
                        @if($reimbursement->is_detail=='1')
                            <li><x-tab :href="route('reimbursements.show', $reimbursement->id).'?tab=detail'" :text="__('modules.reimbursements.detail')" ajax="false" class="detail"/></li>
                        @endif
                    </ul>
                </nav>
            @endif
...
@push('scripts')
    ...
    <script>
        const hasReimbursement = "{!! isset($reimbursement->id)?'1':'0' !!}";
        if (hasReimbursement == '1'){
