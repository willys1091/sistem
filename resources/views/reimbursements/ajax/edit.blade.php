@php
    $addReimbursementCategoryPermission = user()->permission('manage_reimbursement_category');
    $approveReimbursementPermission = user()->permission('approve_reimbursements');
@endphp
        ...
        <x-form id="save-reimbursement-data-form">
            ...
            <div class="add-client bg-white rounded">
                <h4 class="mb-0 p-20 f-21 font-weight-normal text-capitalize border-bottom-grey">
                    @lang('app.reimbursementDetails')</h4>
                <div class="row p-20">
                    <div class="col-md-6 col-lg-4">
                        <x-forms.text class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('modules.reimbursements.itemName')"
                            fieldName="item_name" fieldRequired="true" fieldId="item_name"
                            :fieldPlaceholder="__('placeholders.reimbursement.item')" :fieldValue="$reimbursement->item_name" />
                    </div>

                    <div class="col-md-6 col-lg-2">
                        <input type="hidden" id="currency_id" name="currency_id" value="{{$reimbursement->currency_id}}">
                        <x-forms.select :fieldLabel="__('modules.invoices.currency')" fieldName="currency"
                            fieldRequired="true" fieldId="currency">
                            @foreach ($currencies as $currency)
                                <option @selected($currency->id == $reimbursement->currency_id)
                    ...
                    <div class="col-md-6 col-lg-2">
                        ...
                        fieldName="exchange_rate" fieldRequired="true" :fieldValue="$reimbursement->exchange_rate" :fieldReadOnly="($companyCurrency->id == $reimbursement->currency_id)"
                        :fieldHelp="$reimbursement->currency->currency_name != company()->currency->currency_name ? '( '.$reimbursement->currency->currency_name.' '.__('app.to').' '.company()->currency->currency_name.' )' : ' '"/>
                    </div>
                    <div class="col-md-6 col-lg-4">
                        <x-forms.select fieldId="client_id" fieldName="client_id" :fieldLabel="__('app.client')" search="true">
                            <option value="">--</option>
                            @foreach ($clients as $client)
                                <option @selected($client->id == $reimbursement->client_id) value="{{ $client->id }}">{{ $client->company_name }}</option>
                            @endforeach
                        </x-forms.select>
                    </div>
                    <div class="col-md-6 col-lg-4">
                        <x-forms.datepicker fieldId="purchase_date" fieldRequired="true"
                            ...
                            :fieldValue="$reimbursement->purchase_date->format(company()->date_format)" />
                    </div>
                    <div class="col-md-4">
                        ...
                        <x-forms.input-group>
                            <select class="form-control select-picker" name="category_id" id="reimbursement_category_id" data-live-search="true" disabled>
                                <option value="">--</option>
                                @foreach ($categories as $category)
                                    <option @selected($category->id == $reimbursement->category_id) value="{{ $category->id }}">{{ $category->category_name }}</option>
                                @endforeach
                            </select>

                            @if ($addReimbursementCategoryPermission == 'all' || $addReimbursementCategoryPermission == 'added')
                                <x-slot name="append">
                                    <button id="addReimbursementCategory" type="button"
                                        class="btn btn-outline-secondary border-grey"
                                        data-toggle="tooltip" data-original-title="{{__('modules.reimbursementCategory.addReimbursementCategory') }}">@lang('app.add')</button>
                                </x-slot>
                            @endif
                        </x-forms.input-group>
                    </div>
                    <div class="col-md-6 col-lg-3 reimbursement_price">
                        <x-forms.number class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('app.amount')" fieldName="price" fieldRequired="true" fieldId="price" :fieldPlaceholder="__('placeholders.price')" :fieldValue="$reimbursement->price" />
                    </div>
                    <div class="col-md-6 col-lg-4">
                        ...
                        <x-forms.input-group>
                            <select class="form-control select-picker" name="urgency" id="urgency" data-live-search="true" data-size="8">
                                <option @selected($reimbursement->urgency == 'normal') value="normal">Normal</option>
                                <option @selected($reimbursement->urgency == 'urgent') value="urgent">Urgent</option>
                            </select>
                        </x-forms.input-group>
                    </div>
                    <div class="col-md-6 col-lg-4">
                        ...
                        <x-forms.input-group>
                            <select class="form-control select-picker" name="payment_type" id="payment_type" data-live-search="true" data-size="8">
                                <option @selected($reimbursement->payment_type == 'transfer') value="transfer">Transfer</option>
                                <option @selected($reimbursement->payment_type == 'cash') value="cash">Cash</option>
                            </select>
                        </x-forms.input-group>
                    </div>
                    <div class="col-md-4">
                        <x-forms.text :fieldLabel="__('modules.reimbursements.purchaseFrom')" fieldName="purchase_from" fieldId="purchase_from" :fieldPlaceholder="__('placeholders.reimbursement.vendor')" :fieldValue="$reimbursement->purchase_from" />
                    </div>
                    <div class="col-md-4 payType">
                        <x-forms.text :fieldLabel="__('modules.reimbursements.payee')" fieldName="payee" fieldId="payee" :fieldValue="$reimbursement->payee" fieldRequired="true"/>
                    </div>
                    <div class="col-md-6 col-lg-4 payType">
                        <x-forms.number class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('app.menu.bankaccount')" fieldName="bank_account" fieldRequired="true" fieldId="bank_account" :fieldValue="$reimbursement->bank_account"/>
                    </div>
                    <div class="col-md-4 payType">
                        <x-forms.text :fieldLabel="__('modules.reimbursements.bankName')" fieldName="bank_name" fieldId="bank_name" :fieldValue="$reimbursement->bank_name" fieldRequired="true"/>
                    </div>
                            ...
                            <x-forms.input-group>
                                <select class="form-control select-picker" name="user_id" id="user_id" data-live-search="true" data-size="8">
                                    ...
                                    @foreach ($employees as $item)
                                        <x-user-option :user="$item" :selected="$item->id == $reimbursement->user_id" />
                                    @endforeach
                                </select>
                            </x-forms.input-group>
                        </div>
                                ...
                                <x-forms.input-group>
                                    <select class="form-control select-picker" name="project_id" id="project_id" data-live-search="true" data-size="8">
                                        ...
                                        @foreach ($projects as $project)
                                            <option data-currency-id="{{ $project->currency_id }}" @selected($project->id == $reimbursement->project_id) value="{{ $project->id }}">
                    ...
                    <div class="col-md-12">
                        <div class="form-group my-3">
                            ...
                            <div id="description">{!! $reimbursement->description !!}</div>
                            <textarea name="description" id="description-text" class="d-none"></textarea>
                        </div>
                    </div>

                    <div class="col-lg-12">
                        <x-forms.file :fieldLabel="__('app.bill')" fieldName="bill" fieldId="bill" :fieldValue="$reimbursement->bill_url" allowedFileExtensions="txt pdf doc xls xlsx docx rtf png jpg jpeg svg" :popover="__('messages.fileFormat.multipleImageFile')" />
                    </div>

                    <div class="col-lg-3 col-md-6 col-sm-12">
                        @if (!is_null($reimbursement->bill))
                            <x-file-card :fileName="$reimbursement->bill" :dateAdded="$reimbursement->created_at->diffForHumans()">
                                ...
                                <x-slot name="action">
                                    <div class="dropdown ml-auto file-action">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0" aria-labelledby="dropdownMenuLink" tabindex="0">
                                            <a class="cursor-pointer d-block text-dark-grey f-13 py-3 px-3 " target="_blank" href="{{ $reimbursement->bill_url }}">@lang('app.view')</a>
                                        </div>
                                    </div>
                                </x-slot>
                            </x-file-card>
                ...
                <x-forms.custom-field :fields="$fields" :model="$reimbursement"></x-forms.custom-field>
                <x-form-actions>
...
<script>
    $(document).ready(function() {
        ...
        if('{{ $reimbursement->category->is_detail }}' == '1') {
            $('#price').val(0);
        ...
        const dp1 = datepicker('#purchase_date', {
            position: 'bl',
            dateSelected: new Date("{{ str_replace('-', '/', $reimbursement->purchase_date) }}"),
            ...datepickerConfig
        });

        let categoryId = $('#reimbursement_category_id').val();
        let userId = $('#user_id').val();
        getReimbursementCategoryEmp(userId, categoryId);
        ...
        function getReimbursementCategoryEmp(userId, categoryId) {
        ...
        $('#save-reimbursement-form').click(function() {
            ...
            const url = "{{ route('reimbursements.update', $reimbursement->id) }}";
        ...
        $('#addReimbursementCategory').click(function() {
            const url = "{{ route('reimbursementCategory.create') }}";
