<div class="row">
    <div class="col-sm-12">
        <x-cards.data :title="__('app.menu.expenses') . ' ' . __('app.details')" class=" mt-4">
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0"
                            ...
                            @if ($trashBtn && $editExpensePermission == 'all' || ($editExpensePermission == 'added' && user()->id == $expense->added_by))
                                <a class="dropdown-item openRightModal" href="{{ route('expenses.edit', [$expense->id]) }}">@lang('app.edit')</a>
                            @endif
                            @if ($deleteExpensePermission == 'all' || ($deleteExpensePermission == 'added' && user()->id == $expense->added_by))
                                <a class="dropdown-item delete-table-row" href="javascript:;" data-expense-id="{{ $expense->id }}">@lang('app.delete')</a>
                            @endif
                        </div>
                    </div>
                </x-slot> --}}
            ...
            <x-cards.data-row :label="__('modules.expenses.expenseCategory')" :value="$expense->category->category_name ?? '--'" />

            <x-cards.data-row :label="__('app.urgency')" :value="ucwords($expense->urgency) ?? '--'" />

            <x-cards.data-row :label="__('app.paymentType')" :value="ucwords($expense->payment_type) ?? '--'" />
            
            @if($expense->payment_type == 'transfer')
                <x-cards.data-row :label="__('modules.expenses.payee')" :value="ucwords($expense->payee) ?? '--'" />
                <x-cards.data-row :label="__('app.menu.bankaccount')" :value="ucwords($expense->bank_account) ?? '--'" />
                <x-cards.data-row :label="__('modules.expenses.bankName')" :value="ucwords($expense->bank_name) ?? '--'" />
            @endif
            ...
            <x-cards.data-row :label="__('app.client')" :value="(!is_null($expense->client) ? $expense->client->company_name : '--')" />
            ...
            <div class="col-12 px-0 pb-3 d-lg-flex d-md-flex d-block">
                ...
                <p class="mb-0 text-dark-grey f-14">
                    @if (!is_null($expense->bill))
                        <a target="_blank" href="{{ $expense->bill_url }}" class="text-darkest-grey">@lang('app.view') <i class="fa fa-link"></i></a>&nbsp
                        <a href="{{ $expense->bill_url }}" class="text-darkest-grey" download>@lang('app.download')<i class="fa fa-download f-w-500 mr-1 f-11"></i></a>
                    @else
                        --
                    @endif
                </p>
            ...
            <div class="col-12 px-0 pb-3 d-lg-flex d-md-flex d-block">
                ...
                <p class="mb-0 text-dark-grey f-14"><x-employee :user="$expense->user" /></p>
            </div>
            ...
            <div class="col-12 px-0 pb-3 d-lg-flex d-md-flex d-block">
                ...
                <p class="mb-0 text-dark-grey f-14">
                    @if ($expense->status == 'draft')
                        <x-status :value="ucfirst($expense->status)" color="grey" />
                    @elseif ($expense->status == 'active')
                        <x-status :value="ucfirst($expense->status)" color="light-blue" />
                    @elseif ($expense->status == 'on progress')
                        <x-status :value="ucfirst($expense->status)" color="yellow" />
                    @elseif ($expense->status == 'approved')
                        <x-status :value="ucfirst($expense->status)" color="light-green" />
                    @else
                        <x-status :value="ucfirst($expense->status)" color="red" />
                    @endif
                </p>
            </div>
            ...
            @if ($expense->status == 'approved')
                <div class="col-12 px-0 pb-3 d-lg-flex d-md-flex d-block">
                    <p class="mb-0 text-lightest f-14 w-30 text-capitalize">@lang('modules.expenses.approvedBy')</p>
                    <p class="mb-0 text-dark-grey f-14"><x-employee :user="$expense->approver" /></p>
                </div>
            @endif
