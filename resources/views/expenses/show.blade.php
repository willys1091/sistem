    ...
    <div class="d-flex d-lg-block filter-box project-header bg-white">
        ...
        <div class="project-menu" id="mob-client-detail">
            ...
            @if(isset($expense->id))
                <nav class="tabs">
                    <ul class="-primary">
                        <li><x-tab :href="route('expenses.show', $expense->id)" :text="__('modules.expenses.overview')" class="overview"/></li>
                        @if($expense->is_detail=='1') 
                            <li><x-tab :href="route('expenses.show', $expense->id).'?tab=detail'" :text="__('modules.expenses.detail')" ajax="false" class="detail"/></li>
                        @endif 
                    </ul>
                </nav>
            @endif
...
@push('scripts')
    ...
    <script>
        const hasExpense = "{!! isset($expense->id)?'1':'0' !!}";
        if (hasExpense == '1'){
