    ...
    @if ($column->userSetting && $column->userSetting->collapsed)
        <div class="minimized rounded bg-additional-grey border-grey mr-3">
            <div class="d-flex mt-4 mx-1 b-p-header align-items-center">
                ...
                <p class="mb-3 mx-0 f-15 text-dark-grey font-weight-bold"><i class="fa fa-circle mb-2 text-red" style="color: {{ $column->label_color }}"></i>{{ $column->slug == 'completed' || $column->slug == 'incomplete' ? __('app.' . $column->slug) : $column->column_name }}</p>
    ...
    @else
        <div class="board-panel rounded bg-additional-grey border-grey mr-3">
            <div class="d-flex m-3 b-p-header">
                <p class="mb-0 f-15 mr-3 text-dark-grey font-weight-bold"><i class="fa fa-circle mr-2 text-yellow"style="color: {{ $column->label_color }}"></i>{{ $column->column_name }}</p>
                <span class="b-p-badge bg-grey f-13 px-2 text-lightest font-weight-bold rounded d-inline-block" id="task-column-count-{{ $column->id }}">{{ $column->tasks_count }}</span>
                <span class="ml-auto d-flex align-items-center">
                    ...
                    @if ($addTaskPermission == 'all' || $addTaskPermission == 'added' || $addStatusPermission == 'all')
                        <div class="dropdown">
                            <button class="btn bg-white btn-lg f-10 px-2 py-1 text-dark-grey text-capitalize rounded  dropdown-toggle ml-3" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            ...
                            <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0" aria-labelledby="dropdownMenuLink" tabindex="0">
                                @if ($addTaskPermission == 'all' || $addTaskPermission == 'added')
                                    <a class="dropdown-item openRightModal" href="{{ route('tasks.create') }}?column_id={{ $column->id }}">@lang('app.addTask')</a>
                                @endif
                                @if ($addStatusPermission == 'all')
                                    <hr class="my-1">
                                    <a class="dropdown-item edit-column" data-column-id="{{ $column->id }}" href="javascript:;">@lang('app.edit')</a>
                                @endif
            ...
            <div class="b-p-body">
                <div class="b-p-tasks" id="drag-container-{{ $column->id }}" data-column-id="{{ $column->id }}">
                    <div class="card rounded bg-white border-grey b-shadow-4 m-1 mb-3 no-task-card move-disable {{ ($column->tasks_count > 0) ? 'd-none' : '' }}">
                        <div class="card-body">
                            <div class="d-flex justify-content-center py-3">
                                <p class="mb-0">
                                    @if ($addTaskPermission == 'all' || $addTaskPermission == 'added')
                                        @if (isset($project))
                                            <a href="{{ route('tasks.create') }}?column_id={{ $column->id }}&task_project_id={{ $project->id }}" class="text-dark-grey openRightModal"><i class="fa fa-plus mr-2"></i>@lang('app.add')@lang('app.task')</a>
                                        @else
                                        <a href="{{ route('tasks.create') }}?column_id={{ $column->id }}" class="text-dark-grey openRightModal"><i class="fa fa-plus mr-2"></i>@lang('app.add') @lang('app.task')</a>
                                        @endif
                                    @else
                    ...
                    @foreach ($column['tasks'] as $task)
                        <x-cards.task-card :draggable="(($changeStatusPermission == 'all' || ($task->project && $task->project->project_admin == user()->id)) ? 'true' : 'false')" :task="$task" />
                    @endforeach
                </div>
                @if ($column->tasks_count > count($column['tasks']))
                    <div class="d-flex m-3 justify-content-center">
                        <a class="f-13 text-dark-grey f-w-500 load-more-tasks" data-column-id="{{ $column->id }}" data-total-tasks="{{ $column->tasks_count }}" href="javascript:;">@lang('modules.tasks.loadMore')</a>
                    </div>
                @endif
            </div>
        </div>
