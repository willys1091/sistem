    ...
    <div class="col-sm-12">
        <x-cards.data :title="__('app.menu.pettycashes') . ' ' . __('app.details')" class=" mt-4">
            ...
            <div class="col-12 px-0 pb-3 d-lg-flex d-md-flex d-block">
                ...
                <p class="mb-0 text-dark-grey f-14">
                    @if (!is_null($pettycash->bill))
                        <a target="_blank" href="{{ $pettycash->bill_url }}" class="text-darkest-grey">@lang('app.view') @lang('app.bill') <i class="fa fa-link"></i></a>&nbsp
            ...
            <div class="col-12 px-0 pb-3 d-lg-flex d-md-flex d-block">
                ...
                <p class="mb-0 text-dark-grey f-14">
                    @if ($pettycash->status == 'draft')
                        <x-status :value="__('app.'.$pettycash->status)" color="grey" />
                    @elseif ($pettycash->status == 'active')
                        <x-status :value="__('app.'.$pettycash->status)" color="light-blue" />
                    @elseif ($pettycash->status == 'on progress')
                        <x-status :value="__('app.'.$pettycash->status)" color="yellow" />
                    @elseif ($pettycash->status == 'approved')
                        <x-status :value="__('app.'.$pettycash->status)" color="light-green" />
                    @else
                        <x-status :value="__('app.'.$pettycash->status)" color="red" />
                    @endif
                </p>
            </div>
            ...
            @if ($pettycash->status == 'approved')
                <div class="col-12 px-0 pb-3 d-lg-flex d-md-flex d-block">
                    <p class="mb-0 text-lightest f-14 w-30 text-capitalize">
                        @lang('modules.pettycashes.approvedBy')</p>
                    <p class="mb-0 text-dark-grey f-14">
                        <x-employee :user="$pettycash->approver" />
                    </p>
                </div>
            @endif
                    ...
                    $.easyAjax({
                        ...
                        success: function(response) {
                            if (response.status == "success") {
                                window.location.href = "{{ route('pettycashes.index')}}";
                            }
                        }
                    });
                }
