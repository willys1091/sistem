...
@php
$addPettycashesPermission = user()->permission('add_pettycashes');
$recurringPettycashesPermission = user()->permission('manage_recurring_pettycash');
@endphp
    ...
    <div class="content-wrapper">
        <div class="d-flex justify-content-between action-bar">
            <div id="table-actions" class="flex-grow-1 align-items-center">
                @if ($addPettycashesPermission == 'all' || $addPettycashesPermission == 'added')
                    <x-forms.link-primary :link="route('pettycashes.create')" class="mr-3 float-left openRightModal"
                        icon="plus">
                        @lang('modules.pettycashes.addPettycash')
                    </x-forms.link-primary>
                @endif
                @if ($recurringPettycashesPermission == 'all')
                    <x-forms.link-secondary :link="route('recurring-pettycashes.index')" class="mr-3 float-left"
                        icon="sync">
                        @lang('app.menu.pettycashesRecurring')
...
@push('scripts')
    ...
    <script>
        $('#pettycashes-table').on('preXhr.dt', function(e, settings, data) {
        ...
        $('body').on('change', '.change-pettycash-status', function() {
            var id = $(this).data('pettycash-id');
            var url = "{{ route('pettycashes.change_status') }}";
            ...
            if (typeof id !== 'undefined') {
                $.easyAjax({
                    url: "{{ route('pettycashes.change_status') }}",
                    type: "POST",
                    data: {
                        '_token': token,
                        pettycashId: id,
        ...
        $('body').on('click', '.open-response', function() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('pettycashes.check_approval') }}",
                data: {'_token': "{{ csrf_token() }}",actid: actid},
                cache: false,
                success: function(response){
                    if (response.status == "success") {
                        var url = "{{ route('pettycashes.response', [':id', ':actid']) }}";
        ...
        $('body').on('click', '.copy', function() {
            var id = $(this).data('pettycash-id');
            $.easyAjax({
                url: "{{ route('pettycashes.copy') }}",
                type: "POST",
                data: {'_token': "{{ csrf_token() }}",pettycashId: id},
                success: function(response) {
                    if (response.status == "success") {
                        showTable();
                    }
        ...
        $('body').on('click', '.delete-table-row', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('pettycashes.destroy', ':id') }}";
                    url = url.replace(':id', id);
        ...
        const applyQuickAction = () => {
            var rowdIds = $("#pettycashes-table input:checkbox:checked").map(function() {
                return $(this).val();
            }).get();

            var url = "{{ route('pettycashes.apply_quick_action') }}?row_ids=" + rowdIds;
