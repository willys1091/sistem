...
@php
$addSettlementsPermission = user()->permission('add_settlements');
@endphp
    ...
    <div class="content-wrapper">
        <div class="d-flex justify-content-between action-bar">
            <div id="table-actions" class="flex-grow-1 align-items-center">
                @if ($addSettlementsPermission == 'all' || $addSettlementsPermission == 'added')
                    {{-- <x-forms.link-primary :link="route('settlements.create')" class="mr-3 float-left openRightModal"
                        icon="plus">
                        @lang('modules.settlements.addSettlement')
                    </x-forms.link-primary> --}}

                    <x-forms.button-primary class="mr-3 float-left" id="add-header" icon="plus" data-redirect-url="{{ route('settlements.index') }}">
                        @lang('modules.settlements.addSettlement')
...
@push('scripts')
    ...
    <script>
        $('body').on('click', '#add-header', function() {
            ...
            var url = "{{ route('settlements.create') }}?redirectUrl="+redirectUrl;
        ...
        $('body').on('click', '.open-edit', function() {
            var id = $(this).data('settlement-id');
            var url = "{{ route('settlements.edit', ':id') }}";
            url = url.replace(':id', id);
            $(MODAL_LG + ' ' + MODAL_HEADING).html('...');
            $.ajaxModal(MODAL_LG, url);
        });

        $('#settlements-table').on('preXhr.dt', function(e, settings, data) {
        ...
        const showTable = () => {
            window.LaravelDataTables["settlements-table"].draw(false);
        }
        ...
        $('body').on('change', '.change-settlements-status', function() {
            var id = $(this).data('settlement-id');
            var url = "{{ route('settlements.change_status') }}";
            ...
            if (typeof id !== 'undefined') {
                $.easyAjax({
                    url: "{{ route('settlements.change_status') }}",
                    type: "POST",
                    data: {
                        '_token': token,
                        settlementId: id,
        ...
        $('body').on('click', '.open-approval', function() {
            var id = $(this).data('settlement-id');
            var url = "{{ route('settlements.approval_list', ':id') }}";
            url = url.replace(':id', id);
            $(MODAL_LG + ' ' + MODAL_HEADING).html('...');
        ...
        $('body').on('click', '.open-response', function() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('settlements.check_approval') }}",
                data: {'_token': "{{ csrf_token() }}",actid: actid},
                cache: false,
                success: function(response){
                    if (response.status == "success") {
                        var url = "{{ route('settlements.response', [':id', ':actid']) }}";
        ...
        $('body').on('click', '.copy', function() {
            var id = $(this).data('settlement-id');
            $.easyAjax({
                url: "{{ route('settlements.copy') }}",
                type: "POST",
                data: {'_token': "{{ csrf_token() }}",settlementId: id},
                success: function(response) {
                    if (response.status == "success") {
                        showTable();
                    }
        ...
        $('body').on('click', '.delete-table-row', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('settlements.destroy', ':id') }}";
                    url = url.replace(':id', id);
        ...
        const applyQuickAction = () => {
            var rowdIds = $("#settlements-table input:checkbox:checked").map(function() {
                return $(this).val();
            }).get();

            var url = "{{ route('settlements.apply_quick_action') }}?row_ids=" + rowdIds;
