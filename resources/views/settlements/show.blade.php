    ...
    <div class="d-flex d-lg-block filter-box project-header bg-white">
        ...
        <div class="project-menu" id="mob-client-detail">
            ...
            @if(isset($settlement->id))  
                <nav class="tabs">
                    <ul class="-primary">
                        <li><x-tab :href="route('settlements.show', $settlement->id)" :text="__('modules.settlements.overview')" class="overview"/></li>
                        @if($settlement->expense->is_detail == "1")
                        <li><x-tab :href="route('settlements.show', $settlement->id).'?tab=detail'" :text="__('modules.settlements.detail')" ajax="false" class="detail"/></li>
                        @endif
                    </ul>
                </nav>
            @endif
...
@push('scripts')
    ...
    <script>
       const hasSettlement = "{!! isset($settlement->id)?'1':'0' !!}";
        if (hasSettlement == '1'){
